    ...
    def test_auto_wrap_policy(self):
        ...
        for policy in FSDP_AUTO_WRAP_POLICY:
            ...
            if policy == "TRANSFORMER_BASED_WRAP":
                env["FSDP_TRANSFORMER_LAYER_CLS_TO_WRAP"] = "BertLayer"
            elif policy == "SIZE_BASED_WRAP":
                env["FSDP_MIN_NUM_PARAMS"] = "2000"
            with mockenv_context(**env):
                fsdp_plugin = FullyShardedDataParallelPlugin()
                if policy == "TRANSFORMER_BASED_WRAP":
                    self.assertEqual(fsdp_plugin.auto_wrap_policy, "TRANSFORMER_BASED_WRAP")
                    self.assertEqual(fsdp_plugin.transformer_layer_cls_to_wrap, BertLayer)
                elif policy == "SIZE_BASED_WRAP":
                    self.assertEqual(fsdp_plugin.auto_wrap_policy, "SIZE_BASED_WRAP")
                    self.assertEqual(fsdp_plugin.min_num_params, 2000)
                else:
                    self.assertEqual(fsdp_plugin.auto_wrap_policy, "NO_AUTO_WRAP")
